 ...
 * */
class IndexingTests extends _testCase.TestCase {
  indexName = 'idx1';
  constructor() {
    super();
  }
  ...
  async testCreateIndex() {
    try {
      const index = _index.IndexBuilder.valueIndex(_index.ValueIndexItem.property('name'), _index.ValueIndexItem.property('documentType'));
      await this.collection.createIndex(this.indexName, index);
      const indexes = await this.collection.indexes();
      (0, _chai.expect)(indexes).to.have.lengthOf(1);
      _chai.assert.equal(indexes[0], this.indexName);
    } catch (error) {
      return {
   ...
   * @returns {Promise<ITestResult>} A promise that resolves to an ITestResult object which contains the result of the verification.
   */
  async testFullTextIndexExpression() {
    try {
      //create index
      const indexName = 'idxFTS';
      const index = _index.IndexBuilder.fullTextIndex(_index.FullTextIndexItem.property('quote')).setIgnoreAccents(true);
      await this.collection.createIndex(indexName, index);

      //validate index created
      const indexes = await this.collection.indexes();
      (0, _chai.expect)(indexes).to.have.lengthOf(1);
      _chai.assert.equal(indexes[0], indexName);
      ...
      (0, _chai.expect)(resultDoc.quote).to.equal('Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat');
    } catch (error) {
      return {
   ...
   * @returns {Promise<ITestResult>} A promise that resolves to an ITestResult object which contains the result of the verification.
   */
  async testCreateSameIndexTwice() {
    try {
      await this.collection.createIndex(this.indexName, _index.IndexBuilder.valueIndex(_index.ValueIndexItem.property('name'), _index.ValueIndexItem.property('documentType')));
      const indexes = await this.collection.indexes();
      (0, _chai.expect)(indexes).to.have.lengthOf(1);
      _chai.assert.equal(indexes[0], this.indexName);
      await this.collection.createIndex(this.indexName, _index.IndexBuilder.valueIndex(_index.ValueIndexItem.property('name'), _index.ValueIndexItem.property('documentType')));
  ...
  async testDeleteIndex() {
    try {
      ...
      (0, _chai.expect)(newIndexes).to.have.lengthOf(0);
    } catch (error) {
      return {
...
exports.IndexingTests = IndexingTests;
//# sourceMappingURL=indexing-test.cjs.map