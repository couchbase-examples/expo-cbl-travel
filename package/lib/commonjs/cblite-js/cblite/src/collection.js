...
class Collection {
  ...
  // 
  _cblListenerTokensByUuid = new Map();
  _cblDocumentListenerTokensByUuid = new Map();
  ...
  constructor(name, scope, database) {
    ...
    Object.defineProperty(this, 'scope', {
      value: scope ?? new _scope.Scope('', database),
      ...propertyOptions
    });
    ...
    this._docListenerTokens = new Map();
  }
  ...
  async fullName() {
    try {
      ...
      throw new Error(`Failed to get fullName for collection '${this.name}': ${error.message}`);
    }
  }
  ...
  async addChangeListener(listener) {
    ...
    if (!this._didStartListener) {
      ...
      const cblListenerToken = new _listenerToken.ListenerToken(uuidToken, async () => {
  ...
  async document(id) {
    ...
    if (docJson && docJson._id) {
      ...
      return Promise.resolve(new _document.Document(retId, sequence, revisionID, this, data));
    } else {
  ...
  async removeChangeListener(token) {
    const uuidToken = typeof token === 'string' ? token // Already a UUID string
