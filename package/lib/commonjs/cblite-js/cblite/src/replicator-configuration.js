...
exports.ReplicatorType = exports.ReplicatorConfiguration = void 0;
var _collectionConfiguration = require("./collection-configuration.js");
var _collectionConfig = require("./collection-config.js");
let ReplicatorType = exports.ReplicatorType = /*#__PURE__*/function (ReplicatorType) {
  ReplicatorType["PUSH_AND_PULL"] = "PUSH_AND_PULL";
  ReplicatorType["PUSH"] = "PUSH";
  ReplicatorType["PULL"] = "PULL";
  return ReplicatorType;
}({});
class ReplicatorConfiguration {
  static ReplicatorType = ReplicatorType;
  ...
  // Made mutable for dual API support

  // OLD API support
  collectionsMap = new Map();
  isNewApi = false;
  static defaultContinuous = false;
  static defaultEnableAutoPurge = true;
  static defaultSelfSignedCertificateOnly = true;
  static defaultAcceptParentDomainCookies = false;
  static defaultAllowReplicatingInBackground = false;
  static defaultHeartbeat = 300;
  static defaultMaxAttemptsSingleShot = 10;
  static defaultMaxAttemptsWaitTime = 300;
   ...
   * ```
   */
  constructor(targetOrCollectionConfigs, target) {
    ...
    if (Array.isArray(targetOrCollectionConfigs)) {
      ...
      this.target = targetOrCollectionConfigs;
    }
   ...
   * @function
   */
  getAllowReplicatingInBackground() {
   ...
   * @function
   */
  setAcceptOnlySelfSignedCerts(selfSignedCerts) {
   ...
   * @function
   */
  setAcceptParentDomainCookies(acceptParentDomainCookies) {
   ...
   * @function
   */
  setAllowReplicatingInBackground(allowReplicatingInBackground) {
   ...
   * @function
   */
  setMaxAttemptWaitTime(maxAttemptWaitTime) {
   ...
   * @function
   */
  setPinnedServerCertificate(pinnedServerCertificate) {
   ...
   * @function
   */
  setReplicatorType(replicatorType) {
  ...
  clone() {
    ...
    if (this.isNewApi) {
      ...
      const clonedCollectionConfigs = this.collectionConfigurations.map(config => {
        const cloned = new _collectionConfiguration.CollectionConfiguration(config.getCollection());
        ...
        if (config.getPushFilter()) {
          ...
          const filterFn = new Function('doc', 'flags', `return (${filterStr})(doc, flags)`);
      ...
      clonedConfig = new ReplicatorConfiguration(clonedCollectionConfigs, this.target);
    } else {
      ...
      for (const [collections, collConfig] of this.collectionsMap.entries()) {
        ...
        let clonedCollConfig;
        if (collConfig) {
          clonedCollConfig = new _collectionConfig.CollectionConfig();
    ...
    clonedConfig.setAcceptOnlySelfSignedCerts(this.getAcceptOnlySelfSignedCerts());
    return clonedConfig;
  }
  ...
  toJson() {
    ...
    config.pinnedServerCertificate = this.pinnedServerCertificate !== undefined ? this.pinnedServerCertificate : '';
    ...
    if (this.isNewApi) {
      ...
      const collectionConfigArray = this.collectionConfigurations.map(collectionConfig => collectionConfig.toJson());
      ...
      if (this.collectionsMap.size === 0) {
        throw new Error('No collections specified in the configuration. Use addCollection() or addCollections() to add collections.');
      }
      const collectionConfigArray = [];
      for (const [collections, collConfig] of this.collectionsMap.entries()) {
   ...
   * @private
   */
  validateCollectionsScopeAndDatabase() {
    ...
    for (const config of this.collectionConfigurations) {
      ...
      if (collection.database.getUniqueName() !== expectedDatabaseName || collection.scope.name !== expectedScopeName) {
        return false;
      }
    }
    return true;
...
exports.ReplicatorConfiguration = ReplicatorConfiguration;
//# sourceMappingURL=replicator-configuration.js.map