...
import { IndexBuilder, ValueIndexItem, FullTextIndexItem } from "../../cblite/index.mjs";
...
export class IndexingTests extends TestCase {
  ...
  async testCreateIndex() {
    try {
      const index = IndexBuilder.valueIndex(ValueIndexItem.property('name'), ValueIndexItem.property('documentType'));
  ...
  async testFullTextIndexExpression() {
    try {
      ...
      const index = IndexBuilder.fullTextIndex(FullTextIndexItem.property('quote')).setIgnoreAccents(true);
  ...
  async testCreateSameIndexTwice() {
    try {
      await this.collection.createIndex(this.indexName, IndexBuilder.valueIndex(ValueIndexItem.property('name'), ValueIndexItem.property('documentType')));
      const indexes = await this.collection.indexes();
      expect(indexes).to.have.lengthOf(1);
      assert.equal(indexes[0], this.indexName);
      await this.collection.createIndex(this.indexName, IndexBuilder.valueIndex(ValueIndexItem.property('name'), ValueIndexItem.property('documentType')));
...
//# sourceMappingURL=indexing-test.mjs.map